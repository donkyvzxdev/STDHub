...
Uma sidebar na lateral esquerda, com um tamanho bom na horizontal e que não seja desconfortavel, e que seja possivel clicar facilmente nos icons. Clicar numa função abre (ou foca) sua aba direto — sem precisar de botão direito. Só o Editor tem painel secundário à direita da sidebar (o explorador); as demais funções não têm painel. As sidebars são móveis (redimensionar/arrastar). Abas de função (Calculadora, Pesquisa, Chatbot) podem ser arrastadas para a doca da direita (ou menu "Ancorar à direita") para uso lado a lado; "Mover para a área principal" traz de volta. Configurações abre só o modal, sem selecionar nada. Sistema de tabs também para o editor (fechar, fechar outras/todas, mover, copiar caminho).
